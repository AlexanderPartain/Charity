...
def retrieve(method, offset, interval, wrapperFile, output):
    global sentinel
    #open image
    image = open(wrapperFile, 'rb')
    ...
    if(method == '-b'):
        ...
        while (pixValue != "" and sentinel != storage):
            ...
            if (byte_count == offset + i*8*interval + len(cur_byte)*interval):
                ...
                if (len(cur_byte) == 8):
                    ...
                    output.write(chr(int(cur_byte, 2))) #write byte to output
        ...
        while (pixValue != "" and sentinel != storage):
            ...
            if (byte_count == offset + i*interval):
                ...
                output.write(pixValue) #write to output
                i += 1
            byte_count += 1
            pixValue = image.read(1)
...
while (testOffset <= offsetMax):
	
	while (testInterval <= intervalMax):

		output = open(outputName, 'w')
		
		# Run the function with the arguments
		retrieve(argMethod, testOffset, testInterval, argWrapperFile, output)
		
		output.close()

		# Increment output name
		outputName = str(int(outputName) + 1)
		
		# Double the interval
		testInterval = testInterval * 2
		
		print "Interval = ", testInterval, "output = ", outputName

	testInterval = 1
	# Double the offset
	testOffset = testOffset * 2
	print "Offset = ", testOffset
